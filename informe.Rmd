...
El dataset en español es de 2019-06-12 (dato obtenido de la [url](https://raw.githubusercontent.com/cienciadedatos/datos-de-miercoles/master/datos/2019/2019-06-12/vinos.csv)), el original en inglés de Keggle fue actualizado por última vez hace 5 años (2018), pero no se indica la fecha exacta de procedencia de los datos.
...
```
Puede observarse que los puntos no bajan de ``r puntos_min``, por lo que la calificación oscila en un rango de ``r puntos_max - puntos_min`` puntos.
...
No hay evidencias de que existan anomalías en los datos, solo algunos valores llamativos, como la diferencia entre la media de precios y los precios máximos, ya que, aunque el máximo es US\$``r precio_max``, solo existen ``r vinos_caros`` (de un total de ``r tamanio_muestra``) que superen los US\$``r round(precio_caro)``(promedio de precio + 3 dasvíos estándar). 
...
Al existir paises con muy pocas reseñas, conviene filtrarlos:

##
(Otros = <0,1% del tamaño de la muestra) 
