...
# Esto lo necesito para poder acceder desde el laburo, tiene un condicional por lo cual solo se ejecuta en ese contexto
...
Se ve que los puntos no bajan de 80, y en base al máximo, mínimo y promedio de precios, que los precios altos tienen mucha dispersión, porque se alejan mucho de la media, con lo que existiría una asimetría hacia la derec
...
p <- vinos |>
  ...
  geom_histogram(binwidth=3, show.legend = FALSE, fill=rainbow(1))
show(p)
```
...
cepas[sort.list(unique(na.omit(vinos$variedad)))]
```
